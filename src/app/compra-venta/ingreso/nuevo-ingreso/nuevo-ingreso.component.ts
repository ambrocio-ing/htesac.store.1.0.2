...
export class NuevoIngresoComponent implements OnInit {
  ...
  constructor(private productoService: ProductoService,
    private ingresoService: IngresoService, private renderer:Renderer2, private loginservice: LoginService) {
  ...
  agregarDI(): void {
    ...
    if (this.isValidDetalleIngreso().length == 0 && this.isValidStock(this.detalleIngreso)) {
      ...
      if (element == undefined || element == null) {        
        ...
        this.renderer.setProperty(vpg, "checked", false);
      }
      else {
        Swal.fire({
          icon: 'info',
  ...
  saveIngreso(): void {
    this.preloader = true;
    this.ingreso.personal.idpersonal = this.loginservice.usuario.id;
